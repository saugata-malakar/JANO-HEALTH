...
- **HF Spaces:** Docker SDK → upload the contents of [`resume-ai-system/deploy/huggingface-space/`](resume-ai-system/deploy/huggingface-space/) plus the [`resume-ai-system/backend/`](resume-ai-system/backend/) folder → push. Live in ~5 minutes (first build pulls PyTorch + downloads embedding model).
...
- The full architecture, scaling story (10k résumés/day), and embedding strategy live in [docs/SYSTEM_DESIGN.md](resume-ai-system/docs/SYSTEM_DESIGN.md).
...
```bash
# clone
git clone https://github.com/saugata-malakar/JANO-HEALTH.git
cd JANO-HEALTH/resume-ai-system
...
|---|---|
| **Architectural soundness** | [docs/SYSTEM_DESIGN.md](resume-ai-system/docs/SYSTEM_DESIGN.md) — modular Pydantic boundaries, single-responsibility engines, async pipeline |
| **Prompt engineering / LLM logic** | [`backend/app/prompts/`](resume-ai-system/backend/app/prompts/) — every LLM call uses an explicit JSON-output prompt; failures fall back to deterministic logic so the pipeline never crashes |
| **Code quality** | Typed throughout (Pydantic + Python 3.12 type hints), docstrings on every module explaining *why*, not just *what* |
| **Problem solving (Kafka ≈ Kinesis)** | [`backend/app/modules/scoring.py:_skill_match`](resume-ai-system/backend/app/modules/scoring.py) — embedding-based semantic matching with kinship rationale |
...
| **Scalability** | [SYSTEM_DESIGN §4](resume-ai-system/docs/SYSTEM_DESIGN.md) — Redis result cache keyed by `sha256(resume_bytes)`, async batch processing, semaphore-bounded LLM concurrency |
